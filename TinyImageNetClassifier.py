...
class Classifier:
    ...
    def eval_on_dataset_v2(self, ckpt_path, dataset_path, training_set=False):
        
        if training_set:
            dataset = dataset_path + "train.txt"
        else:
            dataset = dataset_path + "val.txt"
        ...
        ### TF Records reading ###
        steps_per_epoch = int(np.ceil(len(file_lines) / config.batch_size))
        total_imgs = len(file_lines)
        ...
        with tf.Session() as sess:
            ...
            for b in tqdm(range(steps_per_epoch)):
                if b == steps_per_epoch - 1:
                    batch_lines = file_lines[b * config.batch_size :]
                else:
                    batch_lines = file_lines[b * config.batch_size : (b + 1) * config.batch_size]
                    
                batch_imgs, batch_label = [], []
                for line in batch_lines:
                    line = line.replace('\n', '')
                    path, label_num = line.split(' ')
                    img = cv2.imread(path)
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                    batch_imgs.append(img)
                    
                    label_single = np.zeros(shape=[config.num_classes], dtype=np.float32)
                    label_single[int(label_num)] = 1
                    
                    batch_label.append(label_single)
                batch_imgs = np.array(batch_imgs) / 255.
                batch_label = np.array(batch_label)
                
                t_1, t_5 = sess.run([top_1_correct, top_5_correct], feed_dict={x: batch_imgs, label: batch_label})
                top_1_acc += t_1
                top_5_acc += t_5
